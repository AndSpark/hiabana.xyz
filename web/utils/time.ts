...
export const fromNowInDay = (time: Date | string, current = new Date()) => {
	if (current.getTime() - new Date(time).getTime() < 24 * 60 * 60 * 1000) {
		return relativeTimeFromNow(time, current)
	}
	return parseDate(time, 'YYYY-MM-DD dddd')
}
